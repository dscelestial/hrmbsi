...
if(isset($_POST['submit'])){
   ...
   $update_user = $conn->prepare("UPDATE `tutors` SET status = ? WHERE id = ?");
   ...
   if($status == 'Active'){
      ...
      $phpmailer->send();
      $message[] = 'Tutor Information updated!';  
   }else{
      $msg = "Hello! $email. Your account has been de-activated by the administrator. Account Status: $status
...
<head>
   ...
   <title>HRMBSi | Update Admin Profile</title>
...
<section class="playlist-form">
   ...
   <?php
          $select_admin = $conn->prepare("SELECT * FROM `tutors` WHERE id = ?");
          $select_admin->execute([$get_id]);
          $fetch_admin = $select_admin->fetch(PDO::FETCH_ASSOC);
             $id = $fetch_admin['id'];
             $name = $fetch_admin['name'];
             $email = $fetch_admin['email'];
             $image = $fetch_admin['image'];
             $status = $fetch_admin['status'];
      ?>
   <form class="register" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="col">
            ...
            <p>Email </p>
            <input type="email" name="email" value="<?= $fetch_admin['email']; ?>" maxlength="20"  class="box" readonly>
         </div>
