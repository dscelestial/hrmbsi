...
$select_activity = $conn->prepare("SELECT * FROM `loginactivity` WHERE user_id = ? ORDER BY time DESC");
$select_activity->execute([$email]);
$total_logins = $select_activity->rowCount();
if($total_logins > 0){
$fetchlogin = $select_activity->fetch(PDO::FETCH_ASSOC);
$lastlogin = $fetchlogin['time'];
} else {
$lastlogin = 'No logins yet';
}

$select_likes = $conn->prepare("SELECT * FROM `likes` WHERE user_id = ?");
$select_likes->execute([$get_id]);
...
$select_bookmark->execute([$get_id]);
...
<head>
   ...
   <title>HRMBSi | User Profile</title>
...
<section class="profile">
   ...
   <div class="details">
      ...
      <div class="user">
         ...
         <button type="button" class="inline-btn" data-toggle="modal" data-target="#modules"><i class="fas fa-camera"></i> Modules Completed</button>
   ...
   <div class="details">
      ...
      <div class="box">
            <div class="flex">
               <i class="fas fa-clock"></i><span>Login Activity</span>
               <div>
                  <span>Logins: <?php echo $total_logins . ', ' . 'Last Login: ' . $lastlogin; ?></span>
               </div>
            </div>
         </div>
      </div>
...
<!-- payment modal start -->
 <!-- Modal -->
 <div class="modal fade" id="payments" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <?php
        $users = $conn->query("SELECT * FROM payment WHERE user_id = '$get_id'");
        while ($rowuser = $users->fetch()) {
        $user_id = $rowuser['user_id'];
        $name= $rowuser['name'];
        $payment_type = $rowuser['payment_type'];
        $amount = $rowuser['amount'];   
        $image = $rowuser['image'];    
        $payment_id = $rowuser['payment_id'];     
        ?>
            <tr>
                ...
                <td><img src="../uploaded_files/receipts/<?= $rowuser['image']; ?>" alt="" style="width: 20px; height: 20px;"></td>
            </tr>
        <?php } ?>
        </tbody>
    </table>
...
<!-- ecert modal start -->
 <!-- Modal -->
 <div class="modal fade" id="ecert" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Certificates Completed</h4>
        </div>
        <div class="modal-body">
        <table id="example" class="table table-striped" style="width:100%">
        <thead>
        ...
        <?php
        $users = $conn->query("SELECT * FROM ecert WHERE user_id = '$get_id'");
        while ($rowuser = $users->fetch()) {
        $user_id = $rowuser['user_id'];
        $name= $rowuser['name'];
        ...
        <?php
        $users = $conn->query("SELECT * FROM ecert_zoom WHERE user_id = '$get_id'");
        while ($rowuser = $users->fetch()) {
        $user_id = $rowuser['user_id'];
        $name= $rowuser['name'];
        $attachmentlink = $rowuser['attachmentlink'];
 ...
 <div class="modal fade" id="modules" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <?php
        $users = $conn->query("SELECT c.*, cc.*, u.* FROM content AS c JOIN content_completion AS cc ON c.id = cc.content_id JOIN users AS u ON cc.user_id = u.id WHERE cc.user_id = '$get_id'");
        while ($rowuser = $users->fetch()) {
        $user_id = $rowuser['user_id'];
        $name= $rowuser['name'];
        $title= $rowuser['title'];
