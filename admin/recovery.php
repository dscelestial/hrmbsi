...
if(isset($_POST['submit'])){
   ...
   $select_user = $conn->prepare("SELECT * FROM `tutors` WHERE email = ? LIMIT 1");
   ...
   if($select_user->rowCount() > 0){
     ...
     $msg = "Hello! $email, your pin is $pin.";
     $insert_otp = $conn->prepare("INSERT INTO `otp` (`email`, `pin`,`date`) VALUES (?,?,?)");
     $insert_otp->execute([$email, $pin, $timeactivity]);
...
<head>
   ...
   <title>HRMBSi | Account Recovery</title>
...
<section class="form-container">
   ...
   <form action="" method="post" enctype="multipart/form-data" class="login">
      ...
      <p>Email <span>*</span></p>
      <input type="email" name="email" placeholder="Please enter your account email" required class="box">
      <p class="link">Login your account? <a href="login.php">Login</a></p>
