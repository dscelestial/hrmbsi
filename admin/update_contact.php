...
if(isset($_POST['submit'])){
   ...
   $update_user = $conn->prepare("UPDATE `contact` SET status = ? WHERE id = ?");
   $update_user->execute([$status, $get_id]);
   ...
   $link_folder = '../uploaded_files/ecert/' . $attachment_link;
         ...
         $msg = "Good Day $name, Your feedback has been verified and the certificate has been sent to your email - $email.
         ...
         '<table style="max-width: 600px; margin: 0 auto; padding: 20px;">'
               ...
               .'<tr>'
                  ...
                  .'<p style="font-size: 16px; line-height: 1.5;">Your feedback has been verified and the certificate has been sent to your email:  <b>' . $email . '</b></p>'
...
<section class="form-container">
    ...
    <?php
          ...
          $fetch_tuts = $select_tuts->fetch(PDO::FETCH_ASSOC);
            $id = $fetch_tuts['id'];
            $name = $fetch_tuts['name'];
            $status = $fetch_tuts['status'];
      ?>    
   <form class="register" action="" method="post" enctype="multipart/form-data">
         ...
         <div class="col">
           <p hidden>User ID <span>*</span></p>
            ...
            <p>Event Title <span></span></p>
               ...
               <option value="<?= $fetch_msg['event_title']; ?>" selected><?= $fetch_msg['event_title']; ?></option>
                <?php 
      ...
      <p class="link">Go back? <a href="user_messages.php">Here</a></p>
