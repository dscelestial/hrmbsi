...
if(isset($_POST['delete'])){
   ...
   $verify_playlist = $conn->prepare("SELECT * FROM `zoom` WHERE id = ? LIMIT 1");
   $verify_playlist->execute([$delete_id]);

   if($verify_playlist->rowCount() > 0){

   $delete_zoom = $conn->prepare("DELETE FROM `zoom` WHERE id = ?");
   $delete_zoom->execute([$delete_id]);
      ...
      <div class="box">
        <div><i class="fas fa-circle-dot" style="<?php if($rowzoom['status'] == 'Active'){echo 'color:limegreen'; }else{echo 'color:red';} ?>"></i> <span style="<?php if($rowzoom['status'] == 'Active'){echo 'color:limegreen'; }else{echo 'color:red';} ?>"><?= $rowzoom['status']; ?></span></div>
         ...
         <form action="" method="post" class="flex-btn">
            ...
            <input type="submit" value="delete" class="delete-btn" onclick="return confirm('Delete this link?');" name="delete">
         </form>
      </div>
      <?php } ?>
