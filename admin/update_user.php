...
if(isset($_POST['submit'])){
   ...
   $update_user = $conn->prepare("UPDATE `users` SET status = ? WHERE id = ?");
   $update_user->execute([$status, $get_id]);

   if($status == 'Active'){
      $select_zoom_link = $conn->prepare("SELECT u.*, z.* FROM users AS u JOIN zoom AS z ON u.event_title = z.title WHERE u.id = ?");
      $select_zoom_link->execute([$get_id]);
      $row = $select_zoom_link->fetch(PDO::FETCH_ASSOC);
...
<head>
   ...
   <title>HRMBSi | Update User Profile</title>
...
<section class="playlist-form">
   ...
   <form class="register" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="col">
            <!--<p>Name </p>
            <input type="text" name="name" placeholder="<?= $fetch_user['name']; ?>" maxlength="50"  class="box">-->
            <p>Status </p>
            <select name="status" class="box">
               <option value="<?= $fetch_zoom['status']; ?>" selected><?= $fetch_user['status']; ?></option>
               <option value="Active">Active</option>
               <option value="Inactive">Inactive</option>
            </select>
            <p>Email </p>
            <input type="email" name="email" value="<?= $fetch_user['email']; ?>" maxlength="20"  class="box" readonly>
         </div>
