...
if(isset($_POST['submit'])){
   ...
   $image_folder = '../uploaded_files/receipts/'.$rename;

   $datetoday = date('M-d-Y');

   $msg = "Good Day $name We have received your payment from $payment_type amounting: $amount
   
   Contact us for more details on our number $hnumber or email us at $gmail

   Regards, $hname.";

         $insert_user = $conn->prepare("INSERT INTO `payment`(user_id, name, email, event_title, payment_type, amount, image, payment_id, date) VALUES(?,?,?,?,?,?,?,?,?)");
         $insert_user->execute([$get_id, $name, $email, $event_title, $payment_type, $amount, $rename, $payment_id, $datetoday]);
         ...
         '<table style="max-width: 600px; margin: 0 auto; padding: 20px;">'
               ...
               .'<tr>'
                  ...
                  .'<p style="font-size: 16px; line-height: 1.5;">We have received your payment from: <b> '. $payment_type . '</b> Ammounting:  <b>' . $amount . '</b> </p>'
...
<head>
   ...
   <title>HRMBSi | Add User Payment</title>
   ...
   <form class="register" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="col">
            ...
            <input type="text" name="user_id" value="<?= $fetch_user['id']; ?>" required class="box" readonly>
            <p>Name <span>*</span></p>
            <input type="text" name="name" value="<?= $fetch_user['name']; ?>" required class="box" readonly>
            <p>Email <span>*</span></p>
            <input type="text" name="email" value="<?= $fetch_user['email']; ?>" required class="box" readonly>
            ...
            <select name="payment_type" required class="box">
            ...
            <input type="text" name="amount" placeholder="Enter payment amount" required class="box">
           <p>Insert Receipt Form <span>*</span></p>
     	   <input type="file" name="image" accept="image/*" required class="box">
            </div>
        </div>
      <p class="link">Go back? <a href="users.php">Here</a></p>
