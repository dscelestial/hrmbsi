...
<head>
   ...
   <title>HRMBSi | Update Zoom Link</title>
...
<section class="playlist-form">
   ...
   <?php
          $select_zoom = $conn->prepare("SELECT * FROM `zoom` WHERE id = ?");
          $select_zoom->execute([$get_id]);
          $fetch_zoom = $select_zoom->fetch(PDO::FETCH_ASSOC);
             $id = $fetch_zoom['id'];
             $title = $fetch_zoom['title'];
             $link = $fetch_zoom['link'];
             $password = $fetch_zoom['password'];
             $status = $fetch_zoom['status'];
             $start= $fetch_zoom['start'];
             $end = $fetch_zoom['end'];
             $status = $fetch_zoom['status'];
      ?>
   <form class="register" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="col">
            <p>Event Title </p>
            <input type="text" name="title" value="<?= $fetch_zoom['title']; ?>" maxlength="50"  class="box">
            <p>Event Link </p>
            <input type="text" name="link" value="<?= $fetch_zoom['link']; ?>" maxlength="50"  class="box">
            <p>Event Password </p>
            <input type="text" name="pass" value="<?= $fetch_zoom['password']; ?>" maxlength="50"  class="box">
            <p>Start Time </p>
            ...
            <!--<p>Email </p>
            <input type="email" name="email" placeholder="<?= $fetch_user['email']; ?>" maxlength="20"  class="box">-->
         </div>
